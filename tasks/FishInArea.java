 ...
 * @author Mookyman
 */
public class FishInArea extends Task<ClientContext> {
    ...
    public FishInArea(ClientContext ctx, Paint paint, int fishingSpotID, int equipementCount, String fishingType) {
        super(ctx);
        this.paint = paint;
    ...
    public boolean activate() {
        return !ctx.npcs.select().id(fishingSpotID).isEmpty() // Fishing spot exists nearby
                ...
                && ctx.inventory.select(Variables.FILTER.EQUIPEMENT).count() == equipementCount;
    }

    @Override
    public void execute() {
        Npc fishLocation = ctx.npcs.select().id(fishingSpotID).each(Interactive.doSetBounds(Variables.BOUNDS.FISH)).within(Variables.LOCATION.GUILDDOCK).nearest().poll(); // Set fishing spot
